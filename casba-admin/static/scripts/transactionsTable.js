$(document).ready(function() {
  ...
  table.setAttribute("id", "tableTransactions");
  ...
  for (var i=0; i <= values.length; i++) {
    ...
    for (j in values[i]) {
      if ( j == 0) {
        ...
        cells.innerHTML = '<a href=' + "transaction/" + values[i][j] + '>' + values[i][j] + '</a>';
      }
      else if (j != 0) {
        var cells = tr.insertCell(-1)
  ...
  var divContainer2 = document.getElementById("transactionsTable");
